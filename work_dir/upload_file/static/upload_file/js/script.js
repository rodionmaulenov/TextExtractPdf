...
// Initialize variables to track file upload queue
let filesToUpload = [];
let uploading = false;

// Function to upload a file using AJAX
function uploadFile(file) {
    // If a file is currently being uploaded, add the file to the queue
    if (uploading) {
    ...
    http.onload = () => {
        ...
        // If there are more files in the queue, upload the next one with a 2-second delay
...
// Rest of your existing code for file selection and drag-and-drop
fileSelector.onclick = () => fileSelectorInput.click();
fileSelectorInput.onchange = () => {
    [...fileSelectorInput.files].forEach((file) => {
        if (typeValidation(file.type)) {
